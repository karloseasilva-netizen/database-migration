...
export const Route = createFileRoute("/_authenticated/conta")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Gerencie seus dados pessoais, endereço e preferências na Puro Fio Lingerie.",
      },
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        ...
        <button
          onClick={signOut}
          className="inline-flex items-center gap-2 self-start rounded-full border border-border px-4 py-2 text-sm hover:bg-secondary transition"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-8">
        <Link
          to="/favoritos"
          className="flex items-center gap-3 rounded-xl border border-border bg-background px-4 py-3 hover:bg-secondary transition"
        >
        ...
        >
          ...
          <div>
            <div className="text-xs text-muted-foreground">Meu</div>
        ...
        >
          ...
          <div>
            <div className="text-xs text-muted-foreground">Continuar</div>
            <div className="font-semibold">Comprando</div>
          </div>
        </Link>
      </div>
      ...
      <form onSubmit={save} className="grid gap-6 lg:grid-cols-2">
        <section className="rounded-2xl border border-border bg-background p-6">
          <h2 className="font-serif text-xl text-primary flex items-center gap-2 mb-4">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="col-span-2">
                <Field
                  ...
                  onChange={(v) => setProfile({ ...profile, address_street: v })}
                />
              </div>
              <Field
                label="Número"
                value={profile.address_number ?? ""}
                onChange={(v) => setProfile({ ...profile, address_number: v })}
              />
            </div>
            <Field
              label="Complemento"
              value={profile.address_complement ?? ""}
              onChange={(v) => setProfile({ ...profile, address_complement: v })}
            />
            <div className="grid grid-cols-3 gap-3">
              <div className="col-span-2">
                <Field
                  ...
                  onChange={(v) => setProfile({ ...profile, address_city: v })}
                />
              </div>
              <Field
                label="UF"
                ...
                onChange={(v) =>
                  setProfile({ ...profile, address_state: v.toUpperCase().slice(0, 2) })
                }
              />
            </div>
            <Field
              ...
              onChange={(v) => setProfile({ ...profile, address_zip: v })}
        ...
        <div className="lg:col-span-2 flex justify-end">
          <button
            disabled={saving}
            type="submit"
            className="rounded-full bg-primary px-8 py-3 text-sm font-semibold text-primary-foreground hover:opacity-90 transition disabled:opacity-60"
          >
