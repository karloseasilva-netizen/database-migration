    ...
    <footer className="mt-20 bg-secondary/40 border-t border-border/50">
      <div className="mx-auto max-w-7xl px-4 py-14 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
        <div>
          <img
            src={logoUrl}
            alt={store.name || "Puro Fio Lingerie"}
            className="h-16 w-auto object-contain"
          />
          <p className="mt-5 text-sm text-muted-foreground leading-relaxed">
            Lingeries sofisticadas e confortáveis, feitas para valorizar o seu corpo com a máxima
            delicadeza e luxo cotidiano.
          </p>
        </div>
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-foreground/80">
            ...
            <li>
              <Link to="/produtos" className="hover:text-primary transition">
                Todos os Produtos
              </Link>
            </li>
            <li>
              <Link to="/ofertas" className="hover:text-primary transition">
                Ofertas Especiais
              </Link>
            </li>
            <li>
              <Link to="/favoritos" className="hover:text-primary transition">
                Favoritos
              </Link>
            </li>
            <li>
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm text-foreground/80">
            ...
            {store.cnpj && (
              <li className="text-xs text-muted-foreground mt-2">CNPJ: {store.cnpj}</li>
            )}
            {store.address && (
      ...
      <div className="border-t border-border/50 py-5">
        <div className="mx-auto max-w-7xl px-4 text-center text-xs text-muted-foreground">
          © {new Date().getFullYear()} {store.name || "Puro Fio Lingerie"}. Todos os direitos
          reservados.
        </div>
      </div>
    </footer>
